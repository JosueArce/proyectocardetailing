...
async function sendEmail(booking, fetchImpl) {
  ...
  const response = await fetchImpl('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<h2>Tu cita de detallado fue registrada</h2><p>Hola ${escapeHtml(booking.name)},</p><p>Estos son los datos de la cita:</p><ul><li><strong>Servicio:</strong> ${escapeHtml(booking.service)}</li><li><strong>Vehículo:</strong> ${escapeHtml(booking.vehicle)}</li><li><strong>Fecha:</strong> ${escapeHtml(booking.date)}</li><li><strong>Hora:</strong> ${escapeHtml(booking.time)}</li><li><strong>Costo estimado:</strong> ₡${Number(booking.cost || 0).toLocaleString('es-CR')}</li>${booking.benefitLabel ? `<li><strong>Beneficio:</strong> ${escapeHtml(booking.benefitLabel)}${booking.discount ? ` · Ahorro ₡${Number(booking.discount).toLocaleString('es-CR')}` : ''}</li>` : ''}<li><strong>Pago:</strong> ${booking.paymentMethod === 'sinpe' ? 'SINPE Móvil' : 'Efectivo'} · ${escapeHtml(booking.paymentStatus || 'Pendiente')}</li></ul><p>La reserva permanecerá pendiente hasta que Josue la apruebe.</p>`,
    }),
  })
