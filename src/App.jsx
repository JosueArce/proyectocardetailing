...
function App() {
  ...
  const openBooking = (service) => { setForm(f => ({ ...f, service: service || f.service, name: currentAccount?.name || f.name, email: currentAccount?.email || f.email, phone: currentAccount?.phone || f.phone })); setSubmitted(false); setBookingOpen(true); setMenu(false) }
  const submit = (e) => {
    e.preventDefault()
    const cost = Number(services.find(s => s.name === form.service)?.price.replace('$','') || 0)
    setBookings(prev => [{ ...form, cost, id: Date.now(), status: 'Pendiente', createdAt: new Date().toISOString() }, ...prev])
    setSubmitted(true)
  }
