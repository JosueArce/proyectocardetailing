...
import { ArrowRight, CalendarDays, Car, Check, ChevronRight, Droplets, Headlight, Instagram, Mail, Menu, MessageCircle, PaintShield, Polisher, Seat, ShieldCheck, Sparkles, Spray, Wheel, Window, X } from './icons'
...
const gallery = [
  { url: 'https://images.unsplash.com/photo-1507136566006-cfc505b114fc?auto=format&fit=crop&w=1200&q=85', label: 'Detalle exterior', span: 'wide' },
...
function App() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setBookings(prev => [result.booking || { ...form, cost, paymentStatus: result.paymentStatus || 'Pendiente', calendarEventId: result.eventId, id: Date.now(), status: 'Pendiente', createdAt: new Date().toISOString() }, ...prev])
  ...
  return <>
    ...
    <main>
      ...
      <section className="section intro" id="servicios">
        ...
        {serviceCategories.map(category => <div className="service-category" key={category}><div className="category-heading"><span>{category}</span><i/></div><div className="service-grid">{services.filter(service => service.category === category).map(({ icon: Icon, ...s }) => <article className="service-card" key={s.name}><div className="service-icon"><Icon/></div><div className="service-title"><div><h3>{s.name}</h3><span>{s.category}</span></div><strong><small>valor</small>{s.price}</strong></div><p>{s.desc}</p><ul>{s.features.map(f => <li key={f}><Check size={15}/>{f}</li>)}</ul><button onClick={() => openBooking(s.name)}>Reservar servicio <ArrowRight size={17}/></button></article>)}</div></div>)}
      </section>
