...
export function ServiceCard({ service, contactHref, onContact }) {
  return <article className={`catalog-card ${service.featured ? 'featured-package' : ''}`}>
    ...
    <a className="btn catalog-contact" href={contactHref} target="_blank" rel="noreferrer" onClick={onContact}>Consultar por WhatsApp</a>
  </article>
}

export function AddOn({ addon, contactHref, onContact }) {
  return <article className="addon-card"><span className="addon-icon"><ServiceIcon name={addon.icon}/></span><div><h3>{addon.name}</h3></div><a href={contactHref} target="_blank" rel="noreferrer" onClick={onContact} aria-label={`Consultar por ${addon.name} en WhatsApp`}>Consultar</a></article>
}
